...
**Herdr workspaces + auto-sync del mesh** (2026-07-15): capa `workspace` habilitada (familia `cmd+shift`, nav por picker) y Navigator vim en `cmd+p`. La propagación de config entre workers quedó **automatizada event-driven**: `mesh-fanout.sh` colgado al `ssot-sync push` (PostToolUse) → editar config vía Claude actualiza+recarga herdr en el peer solo (`mu`/`mesh-update` queda como fan-out manual). Fix de fondo: shim `_to` porque `timeout` no existe en macOS y tenía roto el push/pull de `ssot-sync` en la Mac.
...
- **`timeout` no existe en macOS sin coreutils: todo script `shared/` que lo use falla con 127 en la Mac; shim que degrade a correr-sin-límite o usar `gtimeout`.** - source: [herdr workspaces + mesh auto-sync](bitacora/2026-07-15-herdr-workspaces-mesh-autosync.md)
- **La traducción `cmd→prefix` de un multiplexer con passthrough vive en el terminal CLIENTE, no en el host; un host headless solo necesita el config de acciones + reload.** - source: [herdr workspaces + mesh auto-sync](bitacora/2026-07-15-herdr-workspaces-mesh-autosync.md)
- **`cmd+shift+3/4/5` son screenshots de macOS y el OS los intercepta antes que cualquier app — inutilizables como keybinds de terminal.** - source: [herdr workspaces + mesh auto-sync](bitacora/2026-07-15-herdr-workspaces-mesh-autosync.md)
- **Propagación cross-device sin humano: GitHub no entra al tailnet, así que o polleás (timer) o el que pushea avisa a los peers (event-driven, instantáneo pero solo cubre el flujo que dispara el hook).** - source: [herdr workspaces + mesh auto-sync](bitacora/2026-07-15-herdr-workspaces-mesh-autosync.md)
...
| 2026-07-15 | [Herdr workspaces + mesh auto-sync](bitacora/2026-07-15-herdr-workspaces-mesh-autosync.md) | Habilita la capa `workspace` de herdr (familia `cmd+shift`, nav por picker — sin índice por colisión con screenshots macOS) y el Navigator vim en `cmd+p` (j/k, h/l nivel, / buscar, estados). Corta un ping-pong incipiente trackeando `onboarding=false`. Automatiza la propagación cross-mesh: `mesh-update` (`mu`) manual + fan-out event-driven colgado al `ssot-sync push` (PostToolUse) → el peer worker jala+recarga herdr solo. Fix de fondo: shim `_to` porque `timeout` no existe en macOS y tenía roto el push/pull de `ssot-sync` en la Mac. Ship: `scripts/mesh-update.sh`, `scripts/mesh-fanout.sh`, `ssot-sync.sh` (shim + fanout hook), keybinds en `shared/herdr/` + `shared/ghostty/`. |
| 2026-07-20 | [Guías para Arbues: mesh + self-host WorkAdventure](bitacora/2026-07-20-guias-mesh-workadventure.md) | Guías de replicación para Arbues (PDFs branded vía `md2pdf` → Discord): (1) el mesh completo (Tailscale + Herdr + ET + aliases + Termux + Wispr Flow), versionada en `docs/guides/codea-desde-el-celular.md`; (2) self-host de WorkAdventure, aterrizada en el deployment real de André (`~/workadventure` en la Arch, hallado y leído por el propio SSH-mesh — dogfooding). Learning WA: media híbrido gateado por `MAX_USERS_FOR_WEBRTC=4` (≤4 P2P/coturn, >4 LiveKit SFU); `use_external_ip:false` + `TURN_SERVER` vacío = LAN-only, de ahí el "solo funcionó con celus". Ship: `docs/guides/`, `~/workadventure/README-SELFHOST.md` (local en Arch). |
