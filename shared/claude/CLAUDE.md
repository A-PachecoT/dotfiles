...
**La respuesta va primero y sola. Default ≤5 líneas.** Pregunta de sí/no → "sí" o "no", después el porqué si hace falta.
...
## Gates y exit codes
**Nunca pipees un gate** (`make check`, `pytest`, `gh pr checks`, `npm run build`) a `tail`/`grep`/`head`: el pipe devuelve el status del filtro, no del gate, y el rojo se cuela. Corré el gate solo, leé su exit code, filtrá después.

## Entornos y checkouts
- Cada repo de código declara sus entornos en `{repo}/.claude/rules/entornos.md` (URL, rama que despliega, sonda de SHA, regla de promoción). Antes de decir «desplegado», hacer QA o promover a `main`, léelo; si no existe, dilo — no inventes URL. → `handbook/governance/ai-native/PRD-factory-environments-v1.md` §7.5
- Lo auto-cargado (CLAUDE.md, `.claude/rules/*`) es lo del CHECKOUT, no lo del repo: un worktree viejo carga reglas viejas sin avisar. Al entrar a uno: `git rev-list --count HEAD..origin/main` (y `origin/develop` si existe); atrás y sin trabajo propio → `git merge --ff-only` ANTES de actuar. → `core/docs/decision-log.md#2026-09-14-checkout-viejo-reglas-viejas`
