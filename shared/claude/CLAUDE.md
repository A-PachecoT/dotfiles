...
- Cada repo declara sus entornos en `{repo}/.claude/rules/entornos.md`; léelo antes de decir «desplegado», hacer QA o promover. Si no existe, dilo — no inventes URL.
- Lo auto-cargado es del CHECKOUT, no del repo: al entrar a un worktree, `git rev-list --count HEAD..origin/develop` (o `main`); atrás y sin trabajo propio → `git merge --ff-only` antes de actuar. → `core/docs/decision-log.md#2026-09-14-checkout-viejo-reglas-viejas`
